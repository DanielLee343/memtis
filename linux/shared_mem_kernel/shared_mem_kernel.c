...
#define BUFFER_SIZE 4096  // Ensure this is a multiple of page size (typically 4096 bytes)
...
static int __init device_init(void) {
    ...
    shared_buffer = (char *)__get_free_pages(GFP_KERNEL, get_order(BUFFER_SIZE));
    if (!shared_buffer) {
        cdev_del(&c_dev);
        device_destroy(cl, dev_num);
        class_destroy(cl);
